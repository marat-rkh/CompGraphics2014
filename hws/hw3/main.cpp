...
struct program_state {
    ...
    void init() {
        ...
        set_shaders();
        set_draw_configs();
        init_textures(utils::load_texture(TEXTURE_PATH));
        set_texture_filtration();
        set_data_buffer();
    }

    float cur_window_width() { return glutGet(GLUT_WINDOW_WIDTH) / 2; }
    float cur_window_height() { return glutGet(GLUT_WINDOW_HEIGHT); }

    void on_display_event() {
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        // switch to window left part
        glViewport(0, 0, cur_window_width(), cur_window_height());
        glOrtho(0, 0, cur_window_width(), cur_window_height(), -1, 1);
        draw_scene();

        // switch to window right part
        glViewport(cur_window_width(), 0, cur_window_width(), cur_window_height());
        glOrtho(0, 0, cur_window_width(), cur_window_height(), -1, 1);
        draw_scene();
...
private:
    ...
    const char* TEXTURE_PATH = "..//resources//wall2.jpg";
...
//    }
    ...
    void init_textures(texture_data const& tex_data) {
...
//    void make_offscreen_buffers_active() {
...
//    }
    ...
    void draw_scene() {
        ...
        mat4 const proj = perspective(45.0f, cur_window_width() / cur_window_height(), 0.1f, 100.0f);
...
//    void draw_with_filters() {
//        size_t const COMPONENTS = 3; // RGB
//        GLenum const FORMAT = GL_RGB;
//        GLubyte *data = (GLubyte *)malloc(COMPONENTS * WINDOW_HEIGHT * WINDOW_WIDTH);
//        if(data) {
//            glReadPixels(0, 0, WINDOW_WIDTH, WINDOW_HEIGHT, FORMAT, GL_UNSIGNED_BYTE, data);
//        } else {
//            throw msg_exception("glReadPixels has returned empty data");
//        }
//        glBindFramebuffer(GL_FRAMEBUFFER, 0);
//        cur_obj = QUAD;
//        texture_data tex_data = {data, WINDOW_WIDTH, WINDOW_HEIGHT, FORMAT};
//        init_textures(tex_data);
//        set_data_buffer();
//        free(data);
//    }
};
